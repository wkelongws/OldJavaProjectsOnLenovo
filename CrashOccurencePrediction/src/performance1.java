...
public class performance1 extends Configured implements Tool {
	
	public static void main ( String[] args ) throws Exception {
		
		int res = ToolRunner.run(new Configuration(), new performance1(), args);
		System.exit(res); 
	...
	public int run ( String[] args ) throws Exception {
		...
		***/
		...
		// Create the job
		Job job_one = new Job(conf, "performance1 Program Round One"); 
		
		// Attach the job to this Driver
		job_one.setJarByClass(performance1.class); 
		...
		Job job_two = new Job(conf, "performance1 Program Round Two"); 
		job_two.setJarByClass(performance1.class); 
	...
	public static class Reduce_One extends Reducer<Text, Text, NullWritable, Text>  {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) 
				...
				int[] crash_crash = new int[4000];
				int[] crash_nocrash = new int[4000];
				int[] nocrash_crash = new int[4000];
				int[] nocrash_nocrash = new int[4000];
				
				for (Text value : values) {
		 	           String[] v = value.toString().split("\t");
		 	           int real = Integer.parseInt(v[4]);
		 	           double crashlike = Double.parseDouble(v[7]);		 	           
		 	           
		 	          for (int i=-2000;i<2000;i++)
		 				{
		 					double s = Math.exp(i/10.0)/(1.0+Math.exp(i/10.0));
		 					int pred = 0;		 	           
		 	           
		 	           if (crashlike>1)
		 	           {
		 	        	   pred = Integer.parseInt(v[5].replace("\"", ""));
		 	           }
		 	           else if (crashlike>s)
		 	           {
		 	        	   pred = 1;
		 	           ...
		 	           if (real==1 & pred==0)
		 	           {
		 	        	   crash_nocrash[i+2000]++;
		 	           }
		 	           if (real==0 & pred==1)
		 	           {
		 	        	   nocrash_crash[i+2000]++;
		 	           }
		 	           if (real==0 & pred==0)
		 	           {
		 	        	   nocrash_nocrash[i+2000]++;
		 	           }
				...
				context.write(NullWritable.get(), new Text("threshold:"+s+",crash_crash:"+crash_crash[i+2000]
						+",crash_nocrash:"+crash_nocrash[i+2000]+",nocrash_crash:"+nocrash_crash[i+2000]+",nocrash_nocrash:"+nocrash_nocrash[i+2000]));
 				}
