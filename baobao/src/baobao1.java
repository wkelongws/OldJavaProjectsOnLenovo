/**
  *****************************************
  *****************************************
  * Cpr E 419 - Lab 3 - baobao1 - Shuo Wang **
  *****************************************
  *****************************************
  */
...
public class baobao1 extends Configured implements Tool {
	
	public static void main ( String[] args ) throws Exception {
		
		int res = ToolRunner.run(new Configuration(), new baobao1(), args);
		System.exit(res); 
	...
	public int run ( String[] args ) throws Exception {
		...
		String input = "MarchINRIX/reduced_data.txt";    // Input
		String temp = "MarchINRIX/temp1";       // Round one output
		String temp1 = "MarchINRIX/temp2";     // Round two output
		...
		// Create the job
		Job job_one = new Job(conf, "baobao1 Program Round One"); 	
		
		// Attach the job to this Driver
		job_one.setJarByClass(baobao1.class); 
		...
		job_two.setJarByClass(baobao1.class); 
		...
		job_Three.setJarByClass(baobao1.class); 
	...
	public static class Map_One extends Mapper<LongWritable, Text, Text, Text>  {		
		...
		public void map(LongWritable key, Text value, Context context) 
			...
			String length = inputlines[1];
			String speed = inputlines[2];
			String TS = inputlines[inputlines.length-1];
			String Date = TS.split(" ")[0];
			String Time = TS.split(" ")[1];
			...
			if (hh == 0)
			{
				context.write(new Text(XD+","+length+";"+Date+";"+Integer.toString(hh)+","+Integer.toString(hh+1)), 
						new Text(speed+","+Integer.toString(TimeSec)));
			}
			else if (hh == 23)
			{
				context.write(new Text(XD+","+length+";"+Date+";"+Integer.toString(hh-1)+","+Integer.toString(hh)), 
						new Text(speed+","+Integer.toString(TimeSec)));
			}	
			else 
			{
	...
	public static class Reduce_One extends Reducer<Text, Text, Text, Text>  {		
		...
		public void reduce(Text key, Iterable<Text> values, Context context) 
			...
			TreeMap<Integer, Integer> Speeds = new TreeMap<Integer, Integer>();
			TreeMap<Integer, Integer> MovingAvg = new TreeMap<Integer, Integer>();
			
			//Text a = new Text();
			
			int indexfirsthour = 0;
			int indexsecondhour = 0;
			
			String[] kk = key.toString().split(";");
			String XD = kk[0];
			String Date = kk[1];
			String firsthour = kk[2].split(",")[0];
			String secondhour = kk[2].split(",")[1];
			int secsplit = Integer.parseInt(secondhour)*3600;
			...
			for (Text value : values) 
			{
	 	        if (value!=null)
	 	        {
	 	        	...
	 	        	Speeds.put(Integer.parseInt(v[1]), Integer.parseInt(v[0]));	 	        	
	 	        }
	 	           
			}
		
			for (int k : Speeds.keySet()) 
			{
				MovingAvg.put(k, Speeds.get(k));
				if (k-MovingAvg.firstKey()>15*60)
				{
					MovingAvg.remove(MovingAvg.firstKey());
					
					TreeSet<Integer> a = new TreeSet<Integer>();
					
					for (int n : MovingAvg.keySet())
					{
						a.add(MovingAvg.get(n));
					}
					int m = a.size()*90/100;
					List<Integer> b = new ArrayList<>(a);					
					int percentile = b.get(m);
					if (percentile<45)
					{
						if ((MovingAvg.firstKey()+MovingAvg.lastKey())/2<secsplit)
						{
							indexfirsthour = 1;
						}
						else 
			...
			context.write(new Text(XD+";"+Date+";"+firsthour), new Text(Integer.toString(indexfirsthour)));		
			context.write(new Text(XD+";"+Date+";"+secondhour), new Text(Integer.toString(indexsecondhour)));	
